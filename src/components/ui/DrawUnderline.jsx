...
import './DrawUnderline.css'

// All paths cleaned to use currentColor so the stroke color is CSS-controlled.
const SVG_VARIANTS = [
  {
    viewBox: '0 0 310 40',
...
// Global counter so each instance picks the next variant in sequence
let _nextVariantIndex = Math.floor(Math.random() * SVG_VARIANTS.length)

export default function DrawUnderline({ children, delay = 0 }) {
  const wrapperRef = useRef(null)
  const boxRef = useRef(null)
  const hasDrawn = useRef(false)
  // Pick variant index once per instance at creation time
  const variantIndex = useRef(_nextVariantIndex++ % SVG_VARIANTS.length)
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasDrawn.current) {
            ...
            const { viewBox, d } = SVG_VARIANTS[variantIndex.current % SVG_VARIANTS.length]
            ...
            // Animate using stroke-dasharray / stroke-dashoffset (no plugin needed)
  ...
  return (
    <span className="draw-underline-wrapper" ref={wrapperRef}>
      {children}
      <span className="draw-underline-box" ref={boxRef} aria-hidden="true" />
    </span>
  )
}
